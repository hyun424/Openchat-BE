...
@SpringBootTest(properties = {
        ...
        "spring.datasource.url=jdbc:h2:mem:openchat-combined-role-test;MODE=MySQL;DB_CLOSE_DELAY=-1;DATABASE_TO_UPPER=false",
...
class OpenchatCombinedRoleApplicationTests {

    @Test
    void combinedRoleKeepsLegacyApiAndRealtimeCapabilities(ApplicationContext context) {
        assertNotNull(context.getBean(WebSocketConfig.class));
        assertNotNull(context.getBean(ChatWebSocketHandler.class));
        assertNotNull(context.getBean(ChatFanoutService.class));
